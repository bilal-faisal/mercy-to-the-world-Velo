...
const collectionID = "WixForms/d750140e-cb06-47c7-9e6e-1b28567b2be8"

// title from ProgressBar collection
const title = "Projects 2 Template Money"
...
async function updateProgressBarCollection(backendProgress) {
    ...
    $w(`#${amountCollectedIDproject1}`).text = "£" + formatAndTruncate(updatedProgress.progressProject1 + offlinePaymentProject1, 2);
    $w(`#${amountCollectedIDproject2}`).text = "£" + formatAndTruncate(updatedProgress.progressProject2 + offlinePaymentProject2, 2);

    // Update target amounts (textTarget)
    $w(`#${amountTargetproject1}`).text = "£" + formatAndTruncate(targetValues.targetProject1, 2);
    $w(`#${amountTargetproject2}`).text = "£" + formatAndTruncate(targetValues.targetProject2, 2);
    ...
    $w(`#${amountRemainingProject1}`).text = "£" + formatAndTruncate(remaining1, 2);
    $w(`#${amountRemainingProject2}`).text = "£" + formatAndTruncate(remaining2, 2);
